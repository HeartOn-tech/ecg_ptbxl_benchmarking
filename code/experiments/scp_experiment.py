...
class SCP_Experiment():
    ...
    def prepare(self):
        ...
        self.data, self.raw_labels = utils.load_dataset(
            self.data_name, self.datafolder, self.sampling_frequency)
    ...
    def perform(self):
        ...
        modelname = 'ensemble'
        # create ensemble predictions via simple mean across model predictions
        # (except naive predictions)
        ensemblepath = self.outputfolder + \
            self.experiment_name + '/models/' + modelname + '/'
        # create folder for model outputs
        ...
        for model_description in os.listdir(
            ...
            if model_description not in ['ensemble', 'naive']:
        ...
        np.array(ensemble_train).mean(
            axis=0).dump(
            ensemblepath +
            'y_train_pred.npy')
        np.array(ensemble_val).mean(
            axis=0).dump(
            ensemblepath +
            'y_val_pred.npy')
        np.array(ensemble_test).mean(
            axis=0).dump(
            ensemblepath +
            'y_test_pred.npy')
    ...
    def evaluate(
        ...
        if bootstrap_eval:
            ...
            if not dumped_bootstraps:
                for data_type in data_types:
                    sample_inds[data_type] = np.array(
                        utils.get_appropriate_bootstrap_samples(
                            y_labels[data_type], n_bootstraping_samples))
                    # store samples for future evaluations
                    name = utils.data_type_to_name(data_type)
                    sample_inds[data_type].dump(
                        self.outputfolder +
                        self.experiment_name +
                        '/' +
                        name +
        ...
        # sample_inds['test'] = np.array([range(len(y_labels['test']))]) #
        # y_test
        ...
        for m in sorted(
            ...
            for data_type in data_types_ext:
                name = utils.data_type_to_name(data_type)
                y_preds[data_type] = np.load(
                    mpath + 'y_' + name + '_pred.npy', allow_pickle=True)
            ...
            if self.mode == 'estim':
                ...
                for l in label_inds:  # cycle by labels (columns)
                    ...
                    # tp, fp, tn, fn
                    conf_m, y_pred_uniq = utils.conf_matrix(
                        y_labels_col, y_preds_col)
                    df_conf_m = pd.DataFrame(
                        conf_m, columns=['tp', 'fp', 'tn', 'fn'])
                    df_conf_m.insert(0, 'threshold', y_pred_uniq)
                    df_conf_m['FPR'] = conf_m[:, 1] / \
                        (conf_m[:, 1] + conf_m[:, 2])  # fpr = fp / (fp + tn)
                    df_conf_m['FNR'] = conf_m[:, 3] / \
                        (conf_m[:, 0] + conf_m[:, 3])  # fnr = fn / (fn + tp)
                    df_conf_m['PPV'] = conf_m[:, 0] / \
                        (conf_m[:, 0] + conf_m[:, 1])  # ppv = tp / (tp + fp)
                    # roc_curve
                    roc_fpr, roc_tpr, roc_thresholds = metrics.roc_curve(
                        y_labels_col, y_preds_col)
                    ...
                    # det_curve
                    det_fpr, det_fnr, det_thresholds = metrics.det_curve(
                        y_labels_col, y_preds_col)
                    ...
                    roc_dfs.append(
                        pd.DataFrame(
                            roc_res,
                            columns=roc_curve_cols))
                    pr_dfs.append(pd.DataFrame(pr_res, columns=pr_curve_cols))
                    det_dfs.append(
                        pd.DataFrame(
                            det_res,
                            columns=det_curve_cols))

                df_conf_m_res = pd.concat(
                    conf_m_dfs, keys=label_inds, names=[
                        'label', 'i'])
                df_conf_m_res.to_csv(
                    rpath +
                    'train' +
                    '_conf_mat' +
                    '.csv',
                    ';',
                    decimal=',')
                df_roc_res = pd.concat(
                    roc_dfs, keys=label_inds, names=[
                        'label', 'i'])
                df_roc_res.to_csv(
                    rpath +
                    'train' +
                ...
                df_pr_res.to_csv(
                    ...
                    ';',
                    decimal=',')
                df_det_res = pd.concat(
                    det_dfs, keys=label_inds, names=[
                        'label', 'i'])
                df_det_res.to_csv(
                    rpath +
                    'train' +
                ...
                # build graphs
                utils.build_graphs(df_roc_res, df_pr_res)
            else:
                # effectiveness evaluation
                beta1 = 2  # Fbeta parameter
                beta2 = 2  # Gbeta parameter
                # if self.data_name == 'ICBEB':
                # compute classwise thresholds such that recall-focused Fbeta
                # is optimized
                thresholds_Fbeta = utils.find_optimal_cutoff_thresholds_for_Fbeta(
                    y_labels['train'], y_preds['train'], beta1)  # y_train, y_train_pred
